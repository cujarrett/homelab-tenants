...
XR instance files for all workspace apps running on the [homelab](https://github.com/cujarrett/homelab) cluster. Workspace directories are hand-maintained, except the ephemeral `guest-<word>-<word>` sandbox directories (e.g. `guest-quantum-pickle`), which are written and deleted automatically by `launchpad-api` via the GitHub Contents/Git Data API.
